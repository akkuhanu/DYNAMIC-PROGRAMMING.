...
int main(){
    ...
    vector<vec> up(n+1,vec(n+1,-1));
    vec lvl(n+1);

    // LCA --> the lowest level node that is common from
    // path u to root and v to root.

    // level incrases downward.

    // level_lca(u,v) <=min(lvl(v),lvl(u))

    // let say k is the answer and it is x level above you,
    // the x must be geater than 0 , and x <= lvl(u).

    // since we find a range for our answer so we can now perform
    // Binary Search for answer.

    // Lifiting takes Log(N) times.


    // T.C --> O(log(N)^2) --> 
    //     Log(N) for B.S and Log(N) for binary Lifting

    dfs(1,0,lvl,0,tree);
    binary_lifitng(1,0,tree,up);
